...
*/
class Solution {// 贪心，再验证局部最优解
public:
    int findCelebrity(int n) {
        int ans = 0;
        for(int i = 1; i < n; ++i) {
            if(knows(ans, i))// 说明当前的这个celebrity肯定不是名人，因为他认识别的人
                ans = i;
        }
        // 到这里的 celebrity，必定不认识 [celebrity + 1, n - 1]的所有人，也就意味着他后面的人都不可能是名人（他不认识）
        // 他前面的人，全都认识别人，也不可能是名人，所以只有他可能！是名人，还需要验证！
        for(int i = 0; i < n; ++i)
        {
            if(ans == i) continue;
